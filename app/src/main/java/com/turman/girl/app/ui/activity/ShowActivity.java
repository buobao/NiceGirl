        ...
        mToolbar.setOnMenuItemClickListener((item)->{
            switch (item.getItemId()) {
                case R.id.collection_img:
                    ...
                    Snackbar.make(mToolbar,"已收藏",Snackbar.LENGTH_SHORT).show();
                    break;
                case R.id.save_img:
                    if (mTouchImageView.getDrawable() != null) {
                        ...
                        Snackbar.make(mToolbar,"已保存到本地相册",Snackbar.LENGTH_SHORT).show();
                    } else {
                        Snackbar.make(mToolbar,"图片未加载,请稍后再试",Snackbar.LENGTH_SHORT).show();
                    }

                    break;
                case R.id.remove_img:
                    mImageDB.deleteByUrl(mBundle.getString(URL));
                    Snackbar.make(mToolbar,"已移除",Snackbar.LENGTH_SHORT).show();

                    break;
                case R.id.clear_img:
                    mImageDB.clear();
                    Snackbar.make(mToolbar,"已清空收藏除",Snackbar.LENGTH_SHORT).show();
                    break;
            }
            return false;
        });
